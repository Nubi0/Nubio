...
const LoginPage = () => {
  ...
  const login = async (e: any) => {
    e.preventDefault()
    await axios.post('api/v1/member/login', config)
                .then((res) => {
                  console.log(res)
                })
                .catch((err) => {
  ...
  return (
    <LoginPageWrapper>
      ...
      <LoginForm>
        <input type="email" name="id" placeholder="아이디(이메일 형식)" onChange={onChangeId}  />
        ...
        <KakaoLogin src={process.env.PUBLIC_URL + '/assets/kakaologin.png'} onClick={kakaoLogin} />
      </LoginForm>
    </LoginPageWrapper>
  );
};
