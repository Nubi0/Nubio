...
const UserInfo = ({
  ...
  const newNickName = useSelector(
    (state: { profile: { newNickName: string } }) => state.profile.newNickName,
  );
  const isInputDisabled = useSelector(
    (state: { profile: { isInputDisabled: boolean } }) =>
      state.profile.isInputDisabled,
  );
  const email = useSelector(
    (state: { profile: { email: string } }) => state.profile.email,
  );
  const birth = useSelector(
    (state: { profile: { birth: string } }) => state.profile.birth,
  );
  const gender = useSelector(
    (state: { profile: { gender: string } }) => state.profile.gender,
  );
    ...
    <MyInfoWrapper>
      ...
      <div>
        ...
        <div style={{ justifyContent: "space-between" }}>
          ...
          />
          <button onClick={isInputDisabled ? enableInput : doubleCheck}>
            {isInputDisabled ? "수정" : "중복확인"}
          </button>
        </div>
      </div>
      <div>
