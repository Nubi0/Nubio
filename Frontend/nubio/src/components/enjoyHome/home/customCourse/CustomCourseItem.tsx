...
const CustomCourseItem = ({ value }: CourseProps) => {
  ...
  const bookMarkUrl = process.env.PUBLIC_URL + "/assets/course/smallStar.svg";
  const bookMarkFillUrl =
    process.env.PUBLIC_URL + "/assets/course/smallStarFill.svg";
  const url = place_list[0].img_url;
  const noUrl = process.env.PUBLIC_URL + "/assets/course/noImage.png";
  ...
  return (
    <CustomCourseItemWrapper>
      ...
      <TitleWrapper>
        <div>{title}</div>
        <BookMarkImg
          src={favorite_flag ? bookMarkFillUrl : bookMarkUrl}
          alt=""
        />
      </TitleWrapper>
    </CustomCourseItemWrapper>
